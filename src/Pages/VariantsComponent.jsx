...
const VariantsComponent = () => {
    ...
    const onSubmitVariant = (e, id) => {
        ...
        setVariants(prevVal => prevVal.map(eachVar => {
            return eachVar.id === id ? { ...eachVar, isDone: true } : eachVar;
        }));
    };
    const editVariant = (e, id) => {
        setVariants(prevVal => prevVal.map(eachVar => eachVar.id === id ? { ...eachVar, isDone: false } : eachVar))
    }
    console.log(variants.length)
    return (
        <div className='newVariant'>
            {variants.map((variant, id) => <div key={id} className=''>
                <div className=''>
                    {variant.isDone ?
                ...
                <div>
                    {
                        variant.isDone ? (
                            variant.optionVal.map((eachVal, i) => eachVal !== '' &&
                                <div key={i}>
                            ...
                            <div>
                                {variant.optionVal.map((eachVal, i) => (
                                    <div key={i}>
                                        ...
                                        <input id={i} type='text' onChange={(e) => handleOptionVal(e, i, variant.id)} value={eachVal} />
                                    </div>
                                ))}
                            </div>
                        )
                ...
                {variant.isDone ? <button onClick={(e) => editVariant(e, variant.id)}>Edit</button> : <button onClick={(e) => onSubmitVariant(e, variant.id)}>Done</button>}
            </div>)
            }
            {variants.length < 3 && <button onClick={addVariant}>+ Add another option</button>}
        </div>
    );
};
